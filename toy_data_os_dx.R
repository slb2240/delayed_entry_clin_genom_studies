...
survfit(Surv(tt_os_dx_mos, os_status_dx) ~ 1, data = toy_data)
...
survfit(Surv(tt_os_dx_mos, os_status_dx) ~ 1, data = toy_data_subset)

## Calculate median overall survival from diagnosis for subsetted population after left truncation adjustment
survfit(Surv(time = tt_cpt_report_mos, time2 = tt_os_dx_mos, os_status_dx) ~ 1, data = toy_data_subset)
